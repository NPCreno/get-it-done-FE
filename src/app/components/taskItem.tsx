...
export function TaskItem({
  ...
  const { setFieldValue, validateForm, errors, values, setSubmitting } =
    useFormik({
      initialValues,
  ...
  const handleSubmitForm = async (formValues: ISubTaskFormValues) => {
    const validationErrors: FormikErrors<ISubTaskFormValues> =
  ...
  const handleDeleteSub = useCallback(
    async (taskSubInstance_id: string) => {
      // Prevent multiple delete attempts for the same task
      if (isDeleting[taskSubInstance_id]) {
        console.log(
          "Delete already in progress for subtask:",
          taskSubInstance_id
        );
        return;
      }
  ...
  const handleCheckToggle = useCallback(
    async (
      ...
      try {
        ...
        const newStatus =
          type === "task"
            ? task.status === "Complete"
              ...
              : "Complete"
            : subTasks?.find(
                (subtask) => subtask.taskSubInstance_id === subtaskId
              )?.status === "Complete"
            ? "Pending"
            : "Complete";
        ...
        taskUpdateStatus?.(
          `Updating ${itemType.toLowerCase()} status...`,
          "info",
          task
        );
        ...
        if (response?.status !== "success") {
          throw new Error(
            response?.message ||
              `Failed to update ${itemType.toLowerCase()} status`
          );
        }

        // Show success message
        taskUpdateStatus?.(
          `${itemType} ${statusText} successfully!`,
          "success",
          task
        );
      } catch (error) {
  ...
  return (
    <div className="w-full">
      <div
        className={`flex flex-row w-full h-[46px] items-center rounded-[10px] ${
          userData?.theme === "dark"
            ? "hover:bg-gray-900"
            : "hover:bg-[#FAFAFA] "
        } 
          cursor-pointer gap-3 pr-4 pl-3 mb-1 ${
        ...
        <div className="flex items-center justify-center">
          <div className="group w-6 h-6 relative">
            <input
              ...
              disabled={isUpdating}
              aria-label={
                isComplete ? "Mark task as pending" : "Mark task as complete"
              }
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <div className="flex items-center justify-between w-full">
            <div className="flex flex-col items-start">
              <span
                className={`flex-1 font-lato text-4 ${
                  task.status === "Complete"
        ...
        <div className="ml-8 pl-2 border-l-2 border-gray-200">
          ...
          {hasSubtasks &&
            subTasks.map((subtask) => (
              <div
                key={subtask.taskSubInstance_id}
